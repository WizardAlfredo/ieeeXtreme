...
vector<int> findElements(int number, vector<int> num_sequence, int last_answer,
                         int troll_answer) {
    if (number == 0) {
    } else {
...
int main(int argc, const char *argv[]) {
    ...
    while (times <= troll_answers[0] + 2) {
        if (first_time) {
            ...
            final_sequence =
                findElements(times, sequence, troll_answers[times - 1],
                             troll_answers[times]);
            times++;
            first_time = false;
        } else {
            ...
            if (times != seq_num + 1 &&
                troll_answer !=
                    seq_num) // Maybe the answer can be found earlier
            {
                final_sequence = findElements(times, final_sequence,
                                              troll_answers[times - 1],
                                              troll_answers[times]);
                cout << "Q ";
