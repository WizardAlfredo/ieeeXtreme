...
// express the message given in the input as stated above as a decimal, rounded up
...
double calculateDegreesBetweenTwoNodes(double firstDegree, double secondDegree){
...
double computeDistanceOfTwoPoints(int radius, double firstDegree, double secondDegree){
    double degreesBetweenTwoNodes, distanceBetweenTwoNodes = 0.0;

    degreesBetweenTwoNodes = calculateDegreesBetweenTwoNodes(firstDegree, secondDegree);
...
int computeFinalDistance(int radius, vector<double> degrees){
    ...
    for(int i = 0; i < degrees.size() - 1; i++){
        if(degrees[i + 1] != degrees[i]){
            ...
            distanceBetweenTwoNodes = computeDistanceOfTwoPoints(radius, degrees[i], degrees[i + 1]);
...
int findElements(int numOfIterations, string inputs)
{
	...
	string testInputString = inputs.substr(nthSubstr(numOfIterations+1,inputs,"\n")+1,nthSubstr(numOfIterations+2,inputs,"\n")-nthSubstr(numOfIterations+1,inputs,"\n"));
    testInputString = testInputString.substr(0,testInputString.find_first_of("\n"));
    ...
    for(int i = 0; i < newDegrees.size() - 1; i++){
        distance = computeDistanceOfTwoPoints(52, newDegrees[i], newDegrees[i + 1]);
